...
def extract(config):
    ...
    path = os.path.join(config["stagging_dir"], "*.csv.zip")
...
def transform(df):
    """Transforms data to ve ready for loading"""
...
def move_files():
    ...
    stagging_path = os.path.join(config["stagging_dir"],
                                 "*.csv.zip")
    ...
    logging.info("File moving compelted")
...
def downsampling():
    ...
    train_file = os.path.join(config["downsampled_dir"], 
    ...
    test_file = os.path.join(config["downsampled_dir"],
                             config["test_dataset"])
